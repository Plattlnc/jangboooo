// 슬라이더 브랜드 로고("S") — 라이트=검정(logo-black), 다크=흰색(logo-white) 자동 전환.
...
export function BrandLogo({ size = 24, className }: { size?: number; className?: string }) {
  ...
  return (
    <>
      ...
      <img src="/logo-black.png" alt="슬라이더" width={size} height={size} style={dim} className={`${base} block dark:hidden`} />
      {/* eslint-disable-next-line @next/next/no-img-element -- 로컬 브랜드 로고, 고정 크기 */}
