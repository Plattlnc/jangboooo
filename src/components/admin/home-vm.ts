...
export interface AdminHomeVM {
  ...
  status: { label: string; value: string; bmart: string | null; color: string; tileBg: string }[];
  peaks: { label: string; value: string; isMax: boolean }[];
  atRisk: { id: string; name: string; rate: string; bandLabel: string; bandColor: string }[];
  top: { id: string; name: string; completed: string; bmart: string | null; rate: string }[];
...
export function toAdminHomeVM(
  ...
  const status = STATUS_META.map((meta) => {
    ...
    return {
      label: meta.label,
      value: fmtCount(hasBreakdown ? Math.max(0, total - bmart) : total),
