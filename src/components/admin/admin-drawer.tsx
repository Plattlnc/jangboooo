"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { BarChart3, Megaphone, Menu, Wrench, Users, LineChart, Target, Award, type LucideIcon } from "lucide-react";
import { BrandLogo } from "@/components/ui/brand-logo";
import { cn } from "@/lib/cn";

// 관리자 햄버거 드로어 — 하단 플로팅 탭 제거(2026-08-13)로 라이더/지표/목표 이동 + 보조 메뉴.
// 라이더 MenuDrawer 와 동일한 스크림/슬라이드 패턴.

interface DrawerItem {
  label: string;
  href: string;
  icon: LucideIcon;
  tileColor: string;
  tileBg: string;
}

const ITEMS: DrawerItem[] = [
  { label: "라이더", href: "/admin/riders", icon: Users, tileColor: "#3182f6", tileBg: "#e8f1fe" },
  { label: "지표", href: "/admin/metrics", icon: LineChart, tileColor: "#3182f6", tileBg: "#e8f1fe" },
  { label: "목표", href: "/admin/goals", icon: Target, tileColor: "#3182f6", tileBg: "#e8f1fe" },
  { label: "공지사항 관리", href: "/admin/notices", icon: Megaphone, tileColor: "#3182f6", tileBg: "#e8f1fe" },
  { label: "사용 현황", href: "/admin/usage", icon: BarChart3, tileColor: "#3182f6", tileBg: "#e8f1fe" },
  { label: "제휴 정비소 관리", href: "/admin/repair-shops", icon: Wrench, tileColor: "#4e5968", tileBg: "#eef0f3" },
  { label: "신규 등급제 현황", href: "/admin/grade-policy", icon: Award, tileColor: "#4f6af5", tileBg: "#eef1fe" },
];

export function AdminMenuButton() {
  const [open, setOpen] = useState(false);
  const pathname = usePathname();

  useEffect(() => {
    if (!open) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setOpen(false);
    };
    document.addEventListener("keydown", onKey);
    return () => document.removeEventListener("keydown", onKey);
  }, [open]);

  return (
    <>
      <button
        type="button"
        onClick={() => setOpen(true)}
        aria-label="관리자 메뉴 열기"
        className="grid size-10 shrink-0 place-items-center rounded-[13px] bg-jb-card text-jb-ink shadow-[var(--toss-shadow)] transition-transform active:scale-95"
      >
        <Menu size={20} strokeWidth={2.2} />
      </button>

      {/* 스크림 */}
      <div
        aria-hidden="true"
        onClick={() => setOpen(false)}
        className={cn(
          "fixed inset-0 z-[300] bg-scrim transition-opacity duration-200",
          open ? "opacity-100" : "pointer-events-none opacity-0",
        )}
      />
      {/* 패널 */}
      <div
        role="dialog"
        aria-modal="true"
        aria-label="관리자 메뉴"
        className={cn(
          "fixed inset-y-0 left-0 z-[400] flex w-[280px] max-w-[80vw] flex-col bg-white shadow-lg",
          "transition-transform duration-[250ms] ease-[var(--ease-emphasized)]",
          open ? "translate-x-0" : "-translate-x-full",
        )}
      >
        <div className="flex items-center gap-2.5 border-b border-jb-line bg-white px-[18px] pb-[18px] pt-[22px]">
          <div className="grid size-11 shrink-0 place-items-center rounded-[12px] bg-jb-indigo-tint">
            <BrandLogo size={22} />
          </div>
          <div className="flex-1">
            <div className="text-[16px] font-black text-jb-ink">슬라이더 관리자</div>
            <div className="mt-0.5 text-[11.5px] font-medium text-jb-ink-mute">운영 도구</div>
          </div>
        </div>
        <nav className="flex-1 overflow-y-auto px-3 pb-2.5 pt-2">
          {ITEMS.map((it) => {
            const active = pathname.startsWith(it.href);
            const Icon = it.icon;
            return (
              <Link
                key={it.href}
                href={it.href}
                onClick={() => setOpen(false)}
                aria-current={active ? "page" : undefined}
                className={cn(
                  "flex w-full items-center gap-[11px] rounded-[12px] px-2.5 py-[10px]",
                  active ? "bg-jb-indigo-tint" : "bg-transparent",
                )}
              >
                <span
                  className="grid size-9 place-items-center rounded-[10px]"
                  style={{ background: it.tileBg, color: it.tileColor }}
                >
                  <Icon size={18} strokeWidth={1.9} />
                </span>
                <span
                  className={cn(
                    "flex-1 text-left text-sm",
                    active ? "font-black text-jb-indigo" : "font-semibold text-jb-ink-soft",
                  )}
                >
                  {it.label}
                </span>
                {active ? <span className="size-1.5 rounded-full bg-jb-indigo" /> : null}
              </Link>
            );
          })}
        </nav>
      </div>
    </>
  );
}
