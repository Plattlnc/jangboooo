...
- **아키텍처 = shadcn 표준 전면 교체(브릿지 아님).** `next-themes` 사용: `<ThemeProvider attribute="class" defaultTheme="light" enableSystem={false}>`(라이트 기본 정책). `.dark` 클래스 토글·FOUC 방지 모두 next-themes 가 `<html>` 에서 처리. 테마 토글 UI 는 `useTheme()`.
- **기존 `[data-theme]` + 2-Layer `tokens.css` 는 폐기·교체.** 다크 셀렉터는 `.dark` 단일. globals.css 정렬: `@custom-variant dark (&:where(.dark, .dark *));`. data-theme 호환 레이어 두지 않음.
...
@theme inline {
  ...
  /* 폰트 — §3 · §4. "Tossface" 를 맨 앞에 두되 unicode-range(§4 @font-face)로
     이모지 코드포인트만 매칭 → 라틴/한글 본문은 Pretendard 가 렌더(영향 0). */
  --font-sans: "Tossface", "Pretendard Variable", "Pretendard", "Apple SD Gothic Neo",
    -apple-system, system-ui, "Noto Sans KR", sans-serif;
  /* 명시 제어용(특정 span 만 강제 이모지 폰트). 일반적으론 위 --font-sans 로 충분 */
  --font-emoji: "Tossface", "Apple Color Emoji", "Segoe UI Emoji", sans-serif;
}
```

> **셀렉터:** `.dark` 단일(next-themes `attribute="class"`). 기존 `[data-theme]`/2-Layer 토큰은 폐기 — 이 블록이 `src/styles/tokens.css` 전체를 대체한다. globals.css 에 `@import "tailwindcss";` + 위 블록 + `@custom-variant dark (&:where(.dark, .dark *));`.
...
**로딩(frontend) — @font-face 스니펫 (이모지 전용, 본문은 Pretendard 유지):**

```css
/* globals.css — Tossface 는 이모지 코드포인트에만 적용(unicode-range).
   → --font-sans 맨 앞에 "Tossface" 를 둬도 라틴/한글 텍스트는 Pretendard 가 렌더,
     이모지 글리프만 Tossface 로 자동 라우팅된다(본문 영향 0). */
@font-face {
  font-family: "Tossface";
  font-display: swap;
  /* self-host 권장: toss/tossface 릴리스 폰트를 /public/fonts/ 에 두고 버전 고정.
     woff2 우선, 없으면 ttf. CDN 으로 갈 경우 공식 배포 URL 을 frontend 가 확정. */
  src: url("/fonts/Tossface.woff2") format("woff2"),
       url("/fonts/Tossface.ttf")  format("truetype");
  unicode-range:
    U+1F000-1FAFF,  /* 이모지 본체(픽토그램/얼굴/사물 등) */
    U+2600-27BF,    /* 기타 기호·딩뱃 */
    U+2B00-2BEF,    /* 화살표·별 등 */
    U+2190-21FF,    /* 화살표 */
    U+FE00-FE0F,    /* 변이 셀렉터(이모지화) */
    U+1F1E6-1F1FF,  /* 국기(리저널 인디케이터) */
    U+20E3, U+2122, U+2139; /* keycap, ™, ℹ */
}
```

그 다음 `@theme inline` 의 `--font-sans` 맨 앞에 `"Tossface"` 추가(아래 §2 폰트 블록 참조 — unicode-range 덕에 텍스트는 영향 없음). 명시 제어가 필요한 곳만 `font-[var(--font-emoji)]` 로 감싸도 됨. 미로딩 시 OS 기본 이모지로 폴백(레이아웃 0 시프트).

> URL 확정: `toss/tossface` 릴리스의 실제 파일명/확장자(woff2 제공 여부)는 frontend 가 배포처에서 확인 후 `src` 경로/포맷 핀. self-host 가 프라이버시·신뢰성 우위(권장).
...
| 테마 토글(신규) | `switch` 또는 `dropdown-menu` + `next-themes` | `useTheme()` 로 setTheme("light"/"dark"). 클래스 토글·저장·FOUC 는 next-themes 담당. ☀️/🌙 라벨 |
...
2. **§2 토큰 블록으로 `src/styles/tokens.css` 전체 교체**(기존 2-Layer/data-theme 폐기). globals.css = `@import "tailwindcss"` + 토큰 블록 + `@custom-variant dark`. **빌드/대비검수 먼저.**
3. `next-themes` 설치 + `<ThemeProvider attribute="class" defaultTheme="light" enableSystem={false}>` 루트 래핑(layout). `<html suppressHydrationWarning>`.
4. primitives 도입(button/card/badge/tabs/input/label/form/popover/tooltip/sonner/alert/separator/switch/skeleton).
5. 자체 컴포넌트 색 클래스 치환(§7 하단 치환표). 매핑표는 §5.
6. Tossface @font-face(§4) + `--font-sans` 선두 추가 + 이모지 액센트 맵 적용.
7. 테마 토글 UI(`useTheme`, ☀️/🌙). FOUC 는 next-themes 가 처리(별도 스크립트 불필요).
8. 디자인 QA 재실행 — **라이트(기본)·다크 양쪽** 상태/대비(AA)/반응형/이모지 점검.
