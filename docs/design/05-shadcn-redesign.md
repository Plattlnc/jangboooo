...
## 0. ⚠️ 결정 필요 — 라이트 기본 vs 다크 퍼스트

- shadcn 컨벤션은 **라이트가 `:root` 기본 / 다크는 `.dark` 오버라이드**. 본 문서는 그 컨벤션대로 **둘 다** 정의한다.
- 단, cmo 브랜딩 §6 은 **dark-first**(야간·도로·저전력 가독)다. 팔레트는 라이트를 기본 레이어로 두되, **앱은 다크를 기본 적용**(`<html class="dark">`)하길 권장한다 — 즉 "토큰은 라이트 기본, 런타임은 다크 디폴트".
- 토글 UI 추가 시 라이트도 1급 지원(이미 양쪽 팔레트 정의). **이 권장(다크 디폴트 적용)이 OK 인지 team-lead 확인 1줄만 부탁** — 라이트 디폴트로 가도 토큰은 그대로 동작.
- 다크 변형 셀렉터: shadcn 표준은 `.dark` 클래스. 현 코드의 `[data-theme="dark"]`(globals.css `@custom-variant`)를 유지하려면 §2 하단 호환 노트 참조.
...
.dark {
  ...
  --primary-foreground:    oklch(0.16 0.02 265);    /* 어두운 글자 */
  --ring:                  oklch(0.62 0.19 260);

  --success:               oklch(0.70 0.15 162);    /* emerald-500 */
  --success-foreground:    oklch(0.16 0.02 265);
  --warning:               oklch(0.79 0.16 75);     /* amber-500 */
  --warning-foreground:    oklch(0.16 0.02 265);
  --destructive:           oklch(0.65 0.20 25);     /* red-500 */
  --destructive-foreground:oklch(0.16 0.02 265);
...
| `ui/info-tip` | `popover` | **탭 토글 = popover**(M1 해결과 정합). tooltip(hover)은 모바일 부적합 — popover 유지 |
...
신규 도입 권장: `popover`(InfoTip), `sonner`(Toast), `alert`(MotivationBanner), `sheet`(로그인 재설계), 선택 `dropdown-menu`(설정).
...
6. 디자인 QA 재실행(상태/대비/반응형/다크). 라이트 신규 → 라이트 모드 대비도 점검.
...
- 다크 기본 적용 시에도 라이트 토큰 정의는 토글/시스템 대비 준비 완료.
